...
class _CreateContentPageState extends State<CreateContentPage> {
  ...
  List? jenisKonten;
  int? selectedJenisKonten;
  final ImagePicker _picker = ImagePicker();
  File? selectedImage;
  String? _imageUrl;

  TextEditingController judulKontenController = TextEditingController();
  TextEditingController deskripsiKontenController = TextEditingController();
  TextEditingController linkVideoController = TextEditingController();
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      if (otherProvider.dataJenisKonten == null) {
        ...
        await otherProvider.getJenisKonten();
        Navigator.pop(context);
      }

      setState(() {
        jenisKonten = otherProvider.dataJenisKonten;
      });
    });
  }
  ...
  Future<void> _addArticle(BuildContext context) async {
    ...
    int? idPengguna = prefs.getInt('idUser');

    if (selectedJenisKonten == 2) {
      try {
        await _uploadImage();
        Konten konten = Konten(
          judul: judulKontenController.text,
          idJenisKonten: selectedJenisKonten,
          gambarVideo: _imageUrl,
          isiDeskripsi: deskripsiKontenController.text,
          idPengguna: idPengguna,
        );
        bool isSuccess = await _contentProvider.addKonten(konten);
        if (isSuccess) {
          Navigator.pop(context);
          Fluttertoast.showToast(msg: 'Konten Berhasil Ditambahkan');
        } else {
          Fluttertoast.showToast(msg: 'Konten Gagal Ditambahkan');
        }
      } catch (e) {
        print(e);
      }
      ...
      try {
        Konten konten = Konten(
          judul: judulKontenController.text,
          idJenisKonten: selectedJenisKonten,
          gambarVideo: linkVideoController.text,
          isiDeskripsi: deskripsiKontenController.text,
          idPengguna: idPengguna,
        );
        bool isSuccess = await _contentProvider.addKonten(konten);
  ...
  bool isComplete() {
    return selectedJenisKonten != null &&
            judulKontenController.text.isNotEmpty &&
            deskripsiKontenController.text.isNotEmpty &&
            linkVideoController.text.isNotEmpty ||
        selectedImage != null;
  }
